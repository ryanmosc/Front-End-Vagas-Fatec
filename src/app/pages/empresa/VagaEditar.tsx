...
import { Briefcase, Calendar, DollarSign, AlertCircle, ArrowLeft } from 'lucide-react';
import { api } from '../../services/api';
import { VagaCreateDTO, VagaResponseDTO, Curso, Modalidade, TipoVaga } from '../../types';
import { toast } from 'sonner';

const VagaEditar: React.FC = () => {
  ...
  const loadVaga = async () => {
    try {
      const data = await api.get<VagaResponseDTO>(`/vagas/${vagaId}`, true);
      setFormData({
        titulo: data.titulo,
        descricao: data.descricao,
        cursoVaga: data.cursoVaga,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/vagas/${vagaId}`, formData, true);
      toast.success('Vaga atualizada com sucesso!');
      navigate('/empresa/dashboard');
    } catch (error: any) {
      setError(error.message || 'Erro ao atualizar vaga');
